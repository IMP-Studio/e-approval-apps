...
@override
  ...
  void initState() {
    ...
    // Setting the initial value for selectedValueType based on the passed widget's absen type
    selectedValueType = widget.absen['type'];

    updateStateBasedOnType();
    ...
    subtitueCont.text = widget.absen['substitute_name'] ?? '';
    fileName = widget.absen['originalFile'];

    if (widget.absen['leave_detail_id'] != null) {
      selectedValueExclusive = widget.absen['leave_detail_id'].toString();
    } // Ensure you have the correct key
    if (widget.absen['leave_detail_id'] != null) {
      selectedValueEmergency = widget.absen['leave_detail_id'].toString();
    } // Ensure you have the correct key

    print(subtitueCont.text);
    print(_pickedFile);
  }
  ...
  Future updateCuti() async {
    ...
    ..fields['_method'] = 'PUT' 
      ...
      ..fields['substitute_id'] =
          selectedSubtitue ?? widget.absen['substitute_id'].toString()
...
// Explicitly setting file to null for yearly type
    ...
    try {
      ...
      // Log the full request to understand what was sent to the server
      print('Request URL: ${request.url}');
      print('Request Method: ${request.method}');
      print('Request Headers: ${request.headers}');
      print('Request Fields: ${request.fields}'); // To log the data fields
      ...
      print('Exception thrown during the request:');
      print(e);
      if (e is Error) {
        print(e.stackTrace); // StackTrace can provide additional details
      }
    }
  }
...
// Helper function to calculate entry date while skipping weekends
  ...
  bool areInputsValid() {
    ...
    if (selectedValueType == 'yearly') {
      if (_mulaiTanggal!.isBefore(now.add(const Duration(days: 4)))) {
        showSnackbarWarning(
            "Fail...",
            "Pengajuan cuti khusus harus H-2",
            kTextBlocker,
      ...
      body: SafeArea(
        child: ListView(
          ...
          children: [
            ...
            DropdownButtonFormField<String>(
              value: selectedValueType ?? widget.absen['type'],
              isExpanded: true,
              decoration: InputDecoration(
                contentPadding:
                    const EdgeInsets.symmetric(vertical: 15, horizontal: 15),
